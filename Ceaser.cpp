...
char caesar( char c, int k ) //'c' holds the letter to be encrypted or decrypted and 'k' holds the key
{
    ...
    else
    {
        ...
        c = tolower(c); //use lower to keep from having to use two seperate for A..Z a..z
    }
    return c;
}
    ...
    while (choice != 2) {
        ...
        if (choice == 1) {
            ...
            for(unsigned int x = 0; x < input.length(); x++) {
                output += caesar(input[x], key);  // calling the Caesar function, where the actual encryption and decryption takes place
            }
